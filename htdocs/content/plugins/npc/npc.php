...
/**
 * Plugin Name: Nepchure
 * Plugin URI: http://wordpress.beetroot.se
...
defined('NPC_DIR') ? NPC_DIR : define('NPC_DIR', plugin_dir_url(__FILE__));
defined('NPC_TEAL') ? NPC_TEAL : define('NPC_TEAL', '#39cccc');

/**
 * Plugin text domain definition
 */
defined('NPC_TD') ? NPC_TD : define('NPC_TD', 'npc');
...
$vars = [
    'slug' => 'npc',
    'name' => 'Nepchure',
    'namespace' => 'nepchure.test',
    'config' => 'nepchure_config',
];
...
add_action('admin_notices', function () use ($vars) {
    if (!class_exists('\Themosis\Foundation\Application')) {
        printf('<div class="notice notice-error"><p>%s</p></div>', __('This plugin requires the Themosis framework in order to work.', NPC_TD));
    }
    ...
    if (!current_theme_supports($vars['slug']) && current_user_can('switch_themes')) {
        printf('<div class="notice notice-warning"><p>%s<strong>%s</strong></p></div>', __('Your application does not handle the following plugin: ', NPC_TD), $vars['name']);
    }
});
...
container('action')->add('plugins_loaded', function () use ($vars) {
     ...
     * I18n
     */
    load_plugin_textdomain(NPC_TD, false, trailingslashit(dirname(plugin_basename(__FILE__))).'languages');
